    ...
    <div class="container">
        <div class="breadcrumb-content">
            <ul>
                ...
                <li class="active">Child Category / {{ ucfirst($slug) }}</li>
            </ul>
        </div>
    </div>
</div>
        ...
        <div class="row">
            <div class="col-lg-9 order-1 order-lg-2">
                ...
                <div class="shop-products-wrapper">
                    <div class="tab-content" id="ChildProductBody">
                        ...
                        <ul>
                            ...
                            <li class="has-sub"><a href="{{ route('category.product',$category->slug) }}">{{ $category->title }}</a>
                                <ul>
                                    ...
                                    <li><a href="{{ route('sub.cat.pro',$sub_category->slug) }}">{{ $sub_category->title }}</a></li>
                                    @endforeach
                                </ul>
                            </li>
                            @endforeach
...
<script>
    $(document).ready(function(){
        ...
        function loadTable(){
            ...
            $.ajax({
                url:"{{ route('get.childCat.pro') }}",
                data:{slug,slug},
                success:function(data){
                    $('#ChildProductBody').html(data);
                }
            });
        }
        loadTable();
        ...
        $(document).on('change','#product_filter',function(){
            ...
            let slug= $('#catSlug').val();
            $.ajax({
                url:"{{ route('child.cat.pro.filter') }}",
        ...
        function pagination(page){
            let slug= $('#catSlug').val();
            $.ajax({
                url: '/childcate-product?page='+page,
                data:{slug,slug},
                success:function(data){
                    $('#ChildProductBody').html(data);
                }
